...
def extract_video_data(media_data, file_path):
    ...
    if video_track:
        ...
        # Frame rate
        frame_rate = video_track.get('FrameRate')
        if frame_rate:
            try:
                # Handle both string and numeric frame rates
                if isinstance(frame_rate, str):
                    # Extract numeric part from string like "23.976 fps" or "29.970"
                    fr_clean = ''.join(c for c in frame_rate if c.isdigit() or c == '.')
                    if fr_clean:
                        fr_value = float(fr_clean)
                else:
                    fr_value = float(frame_rate)
                
                # Round to reasonable precision and format
                if fr_value == int(fr_value):
                    data['mi_fr'] = f"{int(fr_value)} fps"
                else:
                    data['mi_fr'] = f"{fr_value:.3f} fps".rstrip('0').rstrip('.')
                    # Handle common frame rates
                    if abs(fr_value - 23.976) < 0.01:
                        data['mi_fr'] = "23.976 fps"
                    elif abs(fr_value - 29.97) < 0.01:
                        data['mi_fr'] = "29.97 fps"
                    elif abs(fr_value - 59.94) < 0.01:
                        data['mi_fr'] = "59.94 fps"
